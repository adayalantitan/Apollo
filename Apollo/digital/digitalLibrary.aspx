<%@ Page Title="" Language="C#" MasterPageFile="~/App_MasterPages/ApolloSearchVertSplit.master" AutoEventWireup="true" CodeFile="digitalLibrary.aspx.cs" Inherits="Apollo.digital_digitalLibrary" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="searchCriteriaPanel" Runat="Server">        
    <script type="text/javascript" language="javascript">
        ...
        function RegisterSearchFilterKeyPressHandler() {
            ...
            $addHandler($get('<%=agencySearch.Name.ClientID %>'), "keyup", onAutoCompleteSearchFilterKeyPress);
            $addHandler($get('<%=advertiserSearch.Name.ClientID %>'), "keyup", onAutoCompleteSearchFilterKeyPress);
        ...
        function BindDropDowns() {
            ...
            //Filter Advertisers
            $get('<%=advertiserSearch.DependencyId.ClientID %>').value = ddlCompanyDefaultVal;
            //Filter Agencies
            $get('<%=agencySearch.DependencyId.ClientID %>').value = ddlCompanyDefaultVal;
            //Filter AEs
        ...
        function onCompanyChange(sender, e) {
            ...
            //Filter Advertisers
            $get('<%=advertiserSearch.DependencyId.ClientID %>').value = ddlCompany.value;
            $get('<%=advertiserSearch.DependencyValue.ClientID %>').value = ddlCompany[ddlCompany.selectedIndex].text;
            //Filter Agencies
            $get('<%=agencySearch.DependencyId.ClientID %>').value = ddlCompany.value;
            $get('<%=agencySearch.DependencyValue.ClientID %>').value = ddlCompany[ddlCompany.selectedIndex].text;
            //Filter AEs
            ...
            try {
                $find($get('<%=advertiserSearch.BehaviorID.ClientID %>').value)._contextKey = 'companyId:' + ddlCompany.value;
                $find($get('<%=agencySearch.BehaviorID.ClientID %>').value)._contextKey = 'companyId:' + ddlCompany.value;
        ...
        function ClearSearchFilters() {
            ...
            $get('<%=agencySearch.Id.ClientID %>').value = "";
            $get('<%=agencySearch.Name.ClientID %>').value = "";
            $get('<%=advertiserSearch.Id.ClientID %>').value = "";
            $get('<%=advertiserSearch.Name.ClientID %>').value = "";
        ...
        function BuildSearchParamsHash() {
            ...
            hash["agencySearch.AgencyId"] = $get('<%=agencySearch.Id.ClientID %>').value;
            hash["agencySearch.AgencyName"] = $get('<%=agencySearch.Name.ClientID %>').value;
            hash["advertiserSearch.AdvertiserId"] = $get('<%=advertiserSearch.Id.ClientID %>').value;
            hash["advertiserSearch.AdvertiserName"] = $get('<%=advertiserSearch.Name.ClientID %>').value;
        ...
        function SaveSearchFilters() {
            ...
            checkACField($get('<%=agencySearch.Name.ClientID %>'), $get('<%=agencySearch.Id.ClientID %>'));
            checkACField($get('<%=advertiserSearch.Name.ClientID %>'), $get('<%=advertiserSearch.Id.ClientID %>'));
        ...
        function DownloadSelected() {
            ...
            for (var i = 0; i < selectedIds.length; i++) {
                ...
                outputTable += "<img class='thumbSelected' style='margin:5px;' onclick='HandleImageClick(this,\"" + selectedKV[1] + "\")' src='DigitalLibraryImageHandler.ashx?i=" + id + "&c=" + contractNumber + "&t=1&x=" + ext + "' alt='" + selectedIds[i] + "' id='" + id + "' />";
            }
                ...
                <ul class="searchFilters">
                    ...
                    <li>
                        <div>
                            ...
                            <div class="searchFilterRightColumn">             
                                <uc:searchPopup ID="agencySearch" runat="server" ServiceMethod="GetAgenciesDL"
                    ...
                    <li>
                        <div>
                            ...
                            <div class="searchFilterRightColumn">  
                                <uc:searchPopup ID="advertiserSearch" runat="server" ServiceMethod="GetAdvertisersDL"
